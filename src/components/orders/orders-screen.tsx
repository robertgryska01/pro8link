// File: src/components/orders/orders-screen.tsx
'use client';

import React, { useEffect, useMemo, useRef } from 'react';
import { orderItems } from './orders-placeholder-data';
...
export function OrdersScreen({ onScroll, searchQuery, activeFilters }: OrdersScreenProps) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const top = contentRef.current?.offsetTop
        ? contentRef.current.offsetTop - (64 + 44 + 48)
        : 100;
  ...
  return (
    <main
      ref={contentRef}
      className="relative z-10 pt-48 pb-28 px-4 sm:px-6 lg:px-8"
    >
